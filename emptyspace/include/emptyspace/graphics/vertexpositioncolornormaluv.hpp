...
struct VertexPositionColorNormalUvInstanced
{
	glm::vec3 Position;
	glm::vec3 Color;
	glm::vec3 Normal;
	...
	VertexPositionColorNormalUvInstanced(glm::vec3 const& position, glm::vec3 const& color, glm::vec3 const& normal, glm::vec2 const& uv)
		: Position(position), Color(color), Normal(normal), Uv(uv)
	{
	}
};